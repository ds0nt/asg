import path from 'path'

import redisAdapter from 'then-redis'
import redisConfig from '../../config/redis'

let redis = redisAdapter.createClient(redisConfig)
redis.on("ready", err => console.log(`Redis Connected: ${redis.host}:${redis.port}`))
redis.on("error", err => { throw err })

class Loader {
  constructor() {}
  async getTemplate(name) {
    let key = path.join('template', name)
    let src = await redis.get(key)
    return src
  }
  async listTemplates() {
    return await redis.keys("template*")
  }
  async saveTemplate(name, src) {
    let key = path.join('template', name)
    let result = await redis.set(key, src)
    return result
  }
  async getConfig(name) {
    let key = path.join('config', name)
    let src = await redis.get(key)
    return src
  }
  async listConfigs() {
    return await redis.keys("config*")
  }
  async saveConfig(name, src) {
    let key = path.join('config', name)
    let result = await redis.set(key, src)
    return result
  }
}

export default (new Loader(redis))


// // Simple set, incrby, and get
// db.set('my-key', 1)
// db.incrby('my-key', 5)
// db.get('my-key').then(function (value) {
//   assert.strictEqual(value, 6)
// })

// // Multi-key set/get
// db.mset({ a: 'one', b: 'two' })
// db.mget('a', 'b').then(function (values) {
//   assert.deepEqual(values, [ 'one', 'two' ])
// })

// // Sets
// db.sadd('my-set', 1, 2, 3)
// db.sismember('my-set', 2).then(function (value) {
//   assert.strictEqual(value, 1)
// })

// // Hashes
// let originalHash = { a: 'one', b: 'two' }
// db.hmset('my-hash', originalHash)
// db.hgetall('my-hash').then(function (hash) {
//   assert.deepEqual(hash, originalHash)
// })

// // Transactions
// db.multi()
// db.incr('first-key')
// db.incr('second-key')
// db.exec().then(function (reply) {
//   assert.deepEqual(reply, [ 1, 1 ])
// })

// // Pubsub
// let subscriber = redis.createClient()
// subscriber.on('message', function (channel, message) {
//   console.log('Received message: ' + message)
// })
// subscriber.subscribe('my-channel').then(function () {
//   db.publish('my-channel', 'a message')
// })
